...
SWE-bench Verified evaluation for Codewright — **generation runs locally,
...
```bash
pnpm --filter @codewright/eval gen-key you@example.com   # emails you a key + code
pnpm --filter @codewright/eval verify-key                # verify (follow the prompt)
...
SWE_LIMIT=5 pnpm --filter @codewright/eval eval

# re-score an existing predictions file without regenerating
pnpm --filter @codewright/eval score predictions.jsonl
```

`eval` does: load N SWE-bench Verified instances → drive Codewright's
