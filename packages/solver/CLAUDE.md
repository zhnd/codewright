...
`solver` = strategy (*what* to do to fix a bug). `@torin/agent-runtime` =
...
- **`solver` MUST NOT import `@anthropic-ai/claude-agent-sdk`.** All SDK
  contact lives in `@torin/agent-runtime`. Agents call `runAgent` and use
  the sandbox tool config from `@torin/agent-runtime`.
...
src/
  agents/<name>/
    index.ts     # ~20 lines: calls runAgent() from @torin/agent-runtime
...
1. Define the output schema in `@torin/domain/agent-outputs/<name>.ts` (zod).
2. `src/agents/<name>/prompts.ts` — prompt builders (strings only).
3. `src/agents/<name>/index.ts` — import the schema from `@torin/domain`,
   call `runAgent<Result>({ schema, ... })` from `@torin/agent-runtime`.
4. Re-export from `src/index.ts`.

The contract lives in `@torin/domain/agent-outputs/` so consumers
(workflow/server/web) share the same source of truth as the producer.

## Dependencies

- `@torin/agent-runtime` — `runAgent`, sandbox tools, observer (the engine)
- `@torin/domain` — schemas/types (DefectAnalysis, ResolutionResult, …)
- `@torin/sandbox` — `Sandbox` type
- `@torin/shared` — logger
...
`AGENT_MODEL` (resolved inside `@torin/agent-runtime`).
